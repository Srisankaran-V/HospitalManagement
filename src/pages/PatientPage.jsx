// import React from 'react'
// import { useState, useEffect } from 'react'; 
import { Link} from 'react-router-dom';
import { FaArrowLeft, FaLandmark } from 'react-icons/fa';
// import Spinner from '../component/Spinner';
import { useLoaderData } from 'react-router-dom';

const PatientPage = () => {
    // const [loading, setLoading] = useState(true);
    // const [patients, setPatient] = useState([]);
    // const {id} = useParams();
    const patient = useLoaderData();

    

    

    // useEffect(() =>{
    //     const fetchPatients = async () =>{
    //         try{
    //             const res = await fetch(`/api/patient/${id}`);
    //             const data = await res.json();
    //             setPatient(data);
    //         }
    //         catch(err){
    //             console.error( `Fetch Error ${err}`);
    //         }
    //         finally{
    //             setLoading(false);
    //         }
    //     }
    //     fetchPatients();
    // },[]);
   

  return (
    <> 
    {/* <!-- Go Back --> */}
        <section>
        <div className="container m-auto py-6 px-6">
            <Link
            to="/patient"
            className="text-indigo-500 hover:text-indigo-600 flex items-center"
            >
            <FaArrowLeft className='m-2'/> Back to Patients Listings
            </Link>
        </div>
        </section>

        <section className="bg-indigo-50">
        <div className="container m-auto py-10 px-6">
            <div className="grid grid-cols-2 md:grid-cols-60/20 w-full gap-6">
            <main>
                <div
                className="bg-white p-6 rounded-lg shadow-md text-center md:text-left"
                >
                <div className="text-gray-500 mb-4">fulltime</div>
                <h1 className="text-3xl font-bold mb-4">
                    {patient.title}
                </h1>
                <div
                    className="text-gray-500 mb-4 flex align-middle justify-center md:justify-start"
                >
                    <FaLandmark className="text-lg text-orange-700 mr-2" />
                    <p className="text-orange-700">Boston, MA</p>
                </div>
                </div>

                <div className="bg-white p-6 rounded-lg shadow-md mt-6">
                <h3 className="text-indigo-800 text-lg font-bold mb-6">
                    Job Description
                </h3>

                <p className="mb-4">
                We are seeking Link talented Front-End Developer to join our team in Boston, MA. The ideal candidate will have strong skills in HTML, CSS, and JavaScript, with experience working with modern JavaScript frameworks such as React or Angular.
                </p>

                <h3 className="text-indigo-800 text-lg font-bold mb-2">Salary</h3>

                <p className="mb-4">$70k - $80K / Year</p>
                </div>
            </main>

            {/* <!-- Sidebar --> */}
            <aside>
                {/* <!-- Company Info --> */}
                <div className="bg-white p-6 rounded-lg shadow-md">
                <h3 className="text-xl font-bold mb-6">Company Info</h3>

                <h2 className="text-2xl">NewTek Solutions</h2>

                <p className="my-2">
                    NewTek Solutions is Link leading technology company specializing in web development and digital solutions. We pride ourselves on delivering high-quality products and services to our clients while fostering Link collaborative and innovative work environment.
                </p>

                <hr className="my-4" />

                <h3 className="text-xl">Contact Email:</h3>

                <p className="my-2 bg-indigo-100 p-2 font-bold">
                    contact@newteksolutions.com
                </p>

                <h3 className="text-xl">Contact Phone:</h3>

                <p className="my-2 bg-indigo-100 p-2 font-bold">555-555-5555</p>
                </div>

                {/* <!-- Manage --> */}
                <div className="bg-white p-6 rounded-lg shadow-md mt-6">
                <h3 className="text-xl font-bold mb-6">Manage Patient</h3>
                <button className="bg-gray-500 hover:bg-gray-600 text-white font-bold py-2 px-4 rounded-full w-full focus:outline-none focus:shadow-outline mt-4 block">
                    Patient History
                </button>
                <Link
                    to={`/patient/edit/${patient.id}`}
                    className="bg-indigo-500 hover:bg-indigo-600 text-white text-center font-bold py-2 px-4 rounded-full w-full focus:outline-none focus:shadow-outline mt-4 block"
                    >Edit Patient</Link>
                <button
                    className="bg-red-500 hover:bg-red-600 text-white font-bold py-2 px-4 rounded-full w-full focus:outline-none focus:shadow-outline mt-4 block"
                >
                    Delete Patient
                </button>
                </div>
            </aside>
            </div>
        </div>
        </section>
    </>
  )
}
const patientLoader = async ({params})=>{
        const res = await fetch(`/api/patient/${params.id}`);
        const data = await res.json();
        return data;

    };

export { PatientPage as default, patientLoader} ;